...
public class PrecomputedMagicNumbers
{
	...
	public static void populatePEXTArray()
	{
		int rookBase = 0;
		int bishopBase = 0;
		for (int square = 0; square < 64; square++)
		{
			ROOK_MASK[square] = createBlockersMask(square, true);
			//int bits = Long.bitCount(ROOK_MASK[square]);
			//PEXT_ROOK_COMPRESSED_MOVES[square] = new short[1 << bits];
			//PEXT_ROOK_MOVES[square] = new long[1 << bits];
			PEXT_ROOK_MOVES_MASK[square] = getAllPossibleSliderMoves(square, 0, true);
			PEXT_ROOK_INDEX_BASE[square] = rookBase;
			long [] allBlockers = getAllPossibleBlockers(ROOK_MASK[square]);
			for (long blocker : allBlockers)
			{
				int index = (int) Long.compress(blocker, ROOK_MASK[square]);
				long moves = getAllPossibleSliderMoves(square, blocker, true);
				short compressedMoves = (short) Long.compress(moves, PEXT_ROOK_MOVES_MASK[square]);
				PEXT_ROOK_COMPRESSED_MOVES[rookBase + index] = compressedMoves;
				//PEXT_ROOK_MOVES[square][index] = moves;
			}
			rookBase += allBlockers.length;
			
			BISHOP_MASK[square] = createBlockersMask(square, false);
			//bits = Long.bitCount(BISHOP_MASK[square]);
			//PEXT_BISHOP_COMPRESSED_MOVES[square] = new short[1 << bits];
			//PEXT_BISHOP_MOVES[square] = new long[1 << bits];
			PEXT_BISHOP_MOVES_MASK[square] = getAllPossibleSliderMoves(square, 0, false);
			PEXT_BISHOP_INDEX_BASE[square] = bishopBase;
			allBlockers = getAllPossibleBlockers(BISHOP_MASK[square]);
			for (long blocker : allBlockers)
			{
				int index = (int) Long.compress(blocker, BISHOP_MASK[square]);
				long moves = getAllPossibleSliderMoves(square, blocker, false);
				short compressedMoves = (short) Long.compress(moves, PEXT_BISHOP_MOVES_MASK[square]);
				PEXT_BISHOP_COMPRESSED_MOVES[bishopBase + index] = compressedMoves;
				//PEXT_BISHOP_MOVES[square][index] = moves;
			}
			bishopBase += allBlockers.length;
		}
	}
	...
	public static void populateMagicArray()
	{
		for (int i = 0; i < 64; i++)
		{
			ROOK_MASK[i] = createBlockersMask(i, true);
			...
			BISHOP_MASK[i] = createBlockersMask(i, false);
		...
		while (System.currentTimeMillis() < endTime)
		{
			...
			while (count < 1000)
			{
				...
				for (long blockers : blockerCombinations)
				{
					...
					else magicArray[arrayIndex] = getSliderMoves(square, blockers, orthogonal);
				}
				
				if (failed)
				{
					bestNumber = randomLong() & randomLong() & randomLong();
					//while (Long.bitCount(bestNumber) < 6) bestNumber = randomLong() & randomLong() & randomLong();
				}
				else
				{
					if (orthogonal)
	...
	*/
	...
	{
		//return PEXT_ROOK_MOVES[square][(int) Long.compress(blockers, ROOK_MASK[square])];
		return Long.expand(PEXT_ROOK_COMPRESSED_MOVES[PEXT_ROOK_INDEX_BASE[square] + (int) Long.compress(blockers, ROOK_MASK[square])], PEXT_ROOK_MOVES_MASK[square]);
	...
	{
		//return PEXT_BISHOP_MOVES[square][(int) Long.compress(blockers, BISHOP_MASK[square])];
		return Long.expand(PEXT_BISHOP_COMPRESSED_MOVES[PEXT_BISHOP_INDEX_BASE[square] + (int) Long.compress(blockers, BISHOP_MASK[square])], PEXT_BISHOP_MOVES_MASK[square]);
	...
	public static long createBlockersMask(int square, boolean orthogonal)
	{
		long mask = 0;
		int startIndex = orthogonal ? 0 : 4;
		for (int i = startIndex; i < startIndex + 4; i++)
