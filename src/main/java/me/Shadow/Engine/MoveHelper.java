...
public class MoveHelper
{
	...
	{
		if (!isCastleMove(move)) return -1;
		if (getStartIndex(move) < getTargetIndex(move)) return (getStartIndex(move) + 3);
		else return (getStartIndex(move) - 4);
	}
	
	public static int getRookTargetIndex(short move)
	{
		if (!isCastleMove(move)) return -1;
		if (getStartIndex(move) < getTargetIndex(move)) return (getTargetIndex(move) - 1);
		else return (getTargetIndex(move) + 1);
	}
